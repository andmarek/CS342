...
public class Evaluator implements Visitor<Value> {
	...
	public Value visit(AndExp e, Env env) {
		Value.BoolVal f = (Value.BoolVal) e.first_exp().accept(this, env);
		Value.BoolVal s = (Value.BoolVal) e.second_exp().accept(this, env);

		return new Value.BoolVal(f.v() && s.v());
	}
	/* Q3 HW5 */
	...
	public Value visit(IsListExp e, Env env) {
		...
		return new BoolVal((val instanceof Value.PairVal) || (val instanceof Value.Null));
	}
